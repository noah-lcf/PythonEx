...
将word文档转换为可执行SQL，该文档是word文档复制到Notepad++里面得到
...
#             print "append: %s" %line
            ...
            if not  ((re.match(r"^\d+", line) and not re.match(r"^\d\.\d\.\d+", line)) or re.match(r"^(【\d】)|(‘.+’)|(Bit.+).*$", line.encode("utf-8"))):
                print "skip: %s" % line
    return lines2
...
def getSql(tables):
    sql=""
    for t in tables:
        if(t.name.upper().find("HIS")==-1):sql += "\nUSE %s;\n" % USE_DB
        else:sql += "\nUSE %s;\n" % USE_DB_HIS
        ...
        for k in range(0, len(t.indexes)):
            index = t.indexes[k]
            if len(index):sql += "\tKEY `INDEX_%s` (%s)%s\n" % ("_".join([i.upper() for i in index]), ",".join(['`' + i + '`' for i in index]), (k != (len(t.indexes) - 1) and "," or " ")[0]) 
